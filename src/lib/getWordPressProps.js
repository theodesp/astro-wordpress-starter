...
export async function getWordPressProps(options) {
  ...
  const wordPressNodeParams = options.uri;
  resolvedUrl = wordPressNodeParams;
  ...
  const seedQueryRes = await seedQuery(resolvedUrl);
  const seedNode = seedQueryRes?.node;
  if (!seedNode) {
    return new Response(null, {
      status: 404,
      statusText: "Not found",
